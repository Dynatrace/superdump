	...
	public class WindowsTagAnalyzer : DynamicAnalyzer {
		public override void AnalyzeResult(SDResult result) {
			if (result.LastEvent?.Description?.StartsWith("CLR exception") ?? false) {
				result.ThreadInformation.Values.SingleOrDefault(t => t.EngineId == result.LastEvent.ThreadId)?.Tags.Add(SDTag.ManagedExceptionTag);
			} else if (result.LastEvent?.Description?.StartsWith("Access violation") ?? false) {
				result.ThreadInformation.Values.SingleOrDefault(t => t.EngineId == result.LastEvent.ThreadId)?.Tags.Add(SDTag.NativeExceptionTag);
			} else if (result.LastEvent?.Description?.StartsWith("Break instruction exception") ?? false) {
				result.ThreadInformation.Values.SingleOrDefault(t => t.EngineId == result.LastEvent.ThreadId)?.Tags.Add(SDTag.BreakInstructionTag);
			} else if (result.LastEvent?.Description?.StartsWith("Stack overflow") ?? false) {
				result.ThreadInformation.Values.SingleOrDefault(t => t.EngineId == result.LastEvent.ThreadId)?.Tags.Add(SDTag.StackOverflowTag);
			}
		}
